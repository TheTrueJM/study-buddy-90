...
export async function fetchAvailableUnits(studentId = 1) {
    try {
        const [allUnits, studentEnrolments] = await Promise.all([
            fetchUnits(),
            fetch(`${API_BASE}/students/${studentId}/enrolments`).then(r => r.json()).catch(() => [])
        ]);
...
export async function fetchMyGroups(studentId = 1) {
    try {
        ...
        const [allGroups, allMembers] = await Promise.all([
            ...
            fetch(`${API_BASE}/group-requests`).then(r => r.json()).catch(() => []) // Get all group requests to find members
        ]);
...
export async function fetchGroupRequests(studentId = 1) {
    try {
        const response = await fetch(`${API_BASE}/students/${studentId}/group-requests`);
...
export async function deleteGroupRequest(groupId, studentId = 1) {
    try {
        const response = await fetch(`${API_BASE}/group-requests/${groupId}/${studentId}`, {
            method: 'DELETE'
        });
